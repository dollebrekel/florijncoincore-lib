# Contributing to florijncoincore-lib

We're working hard to make *florijncoincore-lib* the most powerful JavaScript library for working with Florijncoin. Our goal is to have *florijncoincore-lib* be a library that can be used by anyone interested in Florijncoin, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://www.florijncoin.org/forum (Support Forum)
...
These are some global design goals in florijncoincore-lib that any change must adhere.
...
When possible, florijncoincore-lib objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of florijncoincore-lib methods. For example, calling `JSON.stringify` on an florijncoincore-lib object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'florijncoincore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the florijncoincore object.

DO:
```javascript
var florijncoincore = require('../');
var PublicKey = florijncoincore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add DolleDev git@github.com:DolleDev/florijncoincore-lib.git
git pull --rebase DolleDev master
```
...
```
Finally go to [github.com/DolleDev/florijncoincore-lib](https://github.com/DolleDev/florijncoincore-lib) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of florijncoincore-lib.

If you have any questions feel free to post them to
[github.com/DolleDev/florijncoincore-lib/issues](https://github.com/DolleDev/florijncoincore-lib/issues).
